<script lang="ts">
  ...
  export interface RelationGraphViewProps {
    /** Knowledge/artifact records. */
    nodes: GraphNode[];
    /** Typed relations between records (conceptually relations[].type). */
    edges: GraphEdge[];
    title?: string;
    nodeCap?: number;
    focusHops?: number;
  ...
  let layoutName: GraphLayoutName = $state("cose");
...
<div class="graph-view">
  ...
  <RelationList
    nodes={visible.nodes}
    edges={visible.edges}
    caption={`${title}: typed relations between records in the current view.`}
  />
</div>
