...
// is NOT a persistence path: canonical queue state lives exclusively in the
// shared SQLite kernel (see syncqueue.go). It touches the old pre-kernel JSONL
...
// of a pre-kernel append-only JSONL sync-queue file, run on first open of the
// queue for a workspace. Before this subsystem moved onto the shared SQLite
// kernel it persisted to <workspaceRoot>/.punakawan/syncqueue/queue.jsonl;
...
// through Append (NOT Enqueue - Enqueue recomputes Attempts/Status/
// ConflictsWith, which would rewrite the imported history) in original file
// order, since Current folds to the latest record per id and later lines must
// win. The renamed file is left in place as a backup and is never deleted.
//
...
// the queue as open and usable. Losing the ability to import old data is much
// better than breaking the ability to open the queue at all going forward.
func (q *Queue) ImportLegacy(workspaceRoot string) error {
	legacyPath := filepath.Join(workspaceRoot, ".punakawan", "syncqueue", "queue.jsonl")
	imported := legacyPath + legacyImportSuffix
	if err := os.Rename(legacyPath, imported); err != nil {
		if os.IsNotExist(err) {
			return nil
		}
		return fmt.Errorf("syncqueue: claim legacy file for import: %w", err)
	}

	f, err := os.Open(imported)
	if err != nil {
		return fmt.Errorf("syncqueue: open imported legacy file %s: %w", imported, err)
	}
	defer f.Close()

	sc := bufio.NewScanner(f)
	// Entries carry adapter params; lift the scanner's line cap well above the
	// 64 KiB default so a large legacy record is not truncated.
	...
	for sc.Scan() {
		...
		if err := json.Unmarshal(raw, &e); err != nil {
			return fmt.Errorf("syncqueue: decode legacy line %d of %s: %w", lineNo, imported, err)
		}
		if err := q.Append(e); err != nil {
			return fmt.Errorf("syncqueue: import legacy entry %q: %w", e.Id, err)
		}
	}
	if err := sc.Err(); err != nil {
		return fmt.Errorf("syncqueue: read imported legacy file %s: %w", imported, err)
	}
	return nil
}
