...
// is NOT a persistence path: canonical proposal state lives exclusively in the
// shared SQLite kernel (see store.go). It touches the old pre-kernel JSONL file
// solely to migrate any data written before the upgrade, then leaves that file
// renamed in place as a backup. The storage-package consolidation guard
...
// of a pre-kernel append-only JSONL proposals file, run on first open of the
// store for a workspace. Before this subsystem moved onto the shared SQLite
// kernel it persisted to <workspaceRoot>/.punakawan/learning/proposals.jsonl;
...
// through Append in original file order (List folds to the latest row per id,
// so later lines must win, which requires importing in the original write
// order). The renamed file is left in place as a backup and is never deleted.
//
...
func (s *Store) ImportLegacy(workspaceRoot string) error {
	legacyPath := filepath.Join(workspaceRoot, ".punakawan", "learning", "proposals.jsonl")
	imported := legacyPath + legacyImportSuffix
	if err := os.Rename(legacyPath, imported); err != nil {
		if os.IsNotExist(err) {
			return nil
		}
		return fmt.Errorf("learning: claim legacy file for import: %w", err)
	}

	f, err := os.Open(imported)
	if err != nil {
		return fmt.Errorf("learning: open imported legacy file %s: %w", imported, err)
	}
	defer f.Close()

	sc := bufio.NewScanner(f)
	// Proposals carry rationale/evidence payloads; lift the scanner's line cap
	// well above the 64 KiB default so a large legacy record is not truncated.
	...
	for sc.Scan() {
		...
		if err := json.Unmarshal(raw, &p); err != nil {
			return fmt.Errorf("learning: decode legacy line %d of %s: %w", lineNo, imported, err)
		}
		if err := s.Append(p); err != nil {
			return fmt.Errorf("learning: import legacy proposal %q: %w", p.Id, err)
		}
	}
	if err := sc.Err(); err != nil {
		return fmt.Errorf("learning: read imported legacy file %s: %w", imported, err)
	}
	return nil
}
