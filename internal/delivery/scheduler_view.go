// scheduler_view.go assembles DeliveryView, the one read model the
...
// DB returns the shared storage kernel this store persists through, so a
// caller that shares the same kernel (e.g. internal/deliveryprojection,
// reaching directly into tables this package and others both write
// through) can query it without this package exposing a duplicate
// per-table accessor for every such caller.
