...
func TestJiraDeliveryCanBindAndLogWork(t *testing.T) {
	...
	// Binding an issue and then recording work against it are two
	// touches of it. Nothing had ever called TouchJiraWorkItem, so
	// touch_count was structurally always 0 and first/last_touched_at
	// always empty - a projected field that could not say anything.
	...
	if item.TouchCount != 2 {
		t.Fatalf("touch_count = %d, want 2 after mapping and logging work against %s", item.TouchCount, item.JiraIssueKey)
	}
