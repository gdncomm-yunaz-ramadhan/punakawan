...
func registerPublicTools(server *mcp.Server, a *app.App, reg *toolIndex, agentReg agent.AgentRegistry) {
	...
	addTool(server, reg, &mcp.Tool{Name: "plan_get", Description: "Fetch a Plan revision. Call this at the start of any planning or execution work to check for an already-saved plan before re-deriving one from scratch."}, planGetHandler(a))
	...
	addTool(server, reg, &mcp.Tool{Name: "finalize_delivery_session", Description: "Close a delivery session exactly once, applying its final usage snapshot atomically."}, finalizeDeliverySessionHandler(a))
