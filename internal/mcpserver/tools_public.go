...
func registerPublicTools(server *mcp.Server, a *app.App, reg *toolIndex, agentReg agent.AgentRegistry) {
	...
	addTool(server, reg, &mcp.Tool{Name: "start_delivery", Description: "Start a delivery from a provider-neutral source (jira | adhoc) or, for one compatibility release, requirement references and project routing."}, startDeliveryHandler(a, agentReg))
