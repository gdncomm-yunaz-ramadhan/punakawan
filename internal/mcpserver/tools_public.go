...
func registerPublicTools(server *mcp.Server, a *app.App, reg *toolIndex) {
	...
	addTool(server, reg, &mcp.Tool{Name: "retry_worklog_sync", Description: "Retry synchronization for one existing delivery worklog without recording it again."}, retryWorklogSyncHandler(a))
