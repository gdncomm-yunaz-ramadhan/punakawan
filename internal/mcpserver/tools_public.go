...
func registerPublicTools(server *mcp.Server, a *app.App, reg *toolIndex) {
	...
	addTool(server, reg, &mcp.Tool{Name: "propose_github_pr_review", Description: "Persist agent review findings and an exact GitHub pull-request review proposal before user confirmation."}, proposeGitHubPRReviewHandler(a))
	...
	addTool(server, reg, &mcp.Tool{Name: "approve_github_pr_review", Description: "Record one explicit user approval for a proposed GitHub pull-request review before submission."}, approveGitHubPRReviewHandler(a))
	addTool(server, reg, &mcp.Tool{Name: "submit_github_pr_review", Description: "Submit one approved GitHub pull-request review through the configured approval-gated GitHub adapter."}, submitGitHubPRReviewHandler(a))
