...
func registerPublicTools(server *mcp.Server, a *app.App, reg *toolIndex, agentReg agent.AgentRegistry) {
	...
	addTool(server, reg, &mcp.Tool{Name: "create_github_pull_request", Description: "Open a GitHub pull request from an existing head branch onto a base branch through the configured adapter. Retrying with the same repository/head/base resolves to the same pull request instead of opening a second one."}, createGitHubPullRequestHandler(a))
