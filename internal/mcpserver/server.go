...
2. start_delivery with source (jira needs tenant and key, or use adhoc), a projects array naming each repository and its tasks, the plan this delivery executes - either plan with its content, or the plan id and revision of one already saved - and a session naming the participant and the worktree path. A plan is required: a delivery is the execution of one, and start_delivery answers with a question instead of a delivery when none is named. Passing plan again on a later call for the same issue saves the next revision and moves the delivery onto it, so a plan that changes stays the delivery's plan. Projects are what create lanes; a session is what makes usage measurable. Passing neither leaves a delivery that can neither run nor be measured. The response carries orchestration_id, execution_id, requirement_sources, the lanes just created, and session.telemetry_session_id; reconciliation.skipped names anything that could not be created and why, and reconciliation.uncovered_requirements names captured requirements no task covers - open work for them, or expect them reported as a gap at step 7. Calling it again for the same source reconciles newly discovered work onto the same delivery rather than starting a second one.
