...
2. External writes (Jira/Confluence edits, comments, attachments, transitions, worklogs - via call_adapter_operation or higher-level tools like update_jira_task_progress, sync_jira_subtasks, request_jira_clarification) are approval-gated per run_id, always, regardless of whether a workflow run or task graph exists for that run_id at all. One human approval covers every approval-required adapter operation during the run. Punakawan first asks the connected MCP client to elicit Approve/Deny inline. If the client does not support form elicitation, the write remains pending: show the error's Approve and Deny choices to the human, never choose for them, and only after their explicit response call respond_to_adapter_approval and retry an approved operation. The CLI commands punakawan approvals approve and punakawan approvals deny remain alternatives. The workflow pipeline above is not required before a one-off write.`
