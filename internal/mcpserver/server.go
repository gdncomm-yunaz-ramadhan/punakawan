...
1. The workflow pipeline (create_workflow_run -> submit_task_graph -> claim_ready_task -> start_task_execution/build_task_context -> submit_lane_bagong_review/submit_semar_synthesis -> finish_task_execution -> commit_task, tracked via get_workflow_state/advance_workflow) is for durable, multi-session/multi-person work: decomposing a large requirement, persisting context and plan/review findings so a later session or teammate doesn't start from zero. It is optional scaffolding, not a prerequisite for anything else - it does not gate approvals or adapter writes in any way.
